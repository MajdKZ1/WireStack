...
func newRootCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "opennet",
		Short: "OpenNET controls local WireGuard configurations",
	}
...
func versionCommand() *cobra.Command {
	return &cobra.Command{
		Use:   "version",
		Short: "Show the OpenNET version",
...
func showServerCommand() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if args[0] != "server" {
				return fmt.Errorf("usage: opennet show server <name>")
			}
			name := args[1]
...
func showClientCommand() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if args[0] != "client" {
				return fmt.Errorf("usage: opennet show client <server> <client>")
			}
			serverName := args[1]
			clientName := args[2]
			profile, err := core.LoadServerProfile(serverName)
...
func connectCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			commandName, args, err := wireGuardInvocation("up", configPath, ninja)
			if err != nil {
				return err
			}
			output, err := utils.RunCommand(commandName, args...)
			if err != nil {
				return err
			}
			if output != "" {
	...
	cmd.Flags().BoolVar(&ninja, "ninja", false, "Wrap wg-quick with Tor (torsocks/torify)")
	return cmd
}
...
func disconnectCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			commandName, args, err := wireGuardInvocation("down", configPath, ninja)
			if err != nil {
				return err
			}
			output, err := utils.RunCommand(commandName, args...)
...
// wireGuardInvocation builds the command and args to run wg-quick, optionally wrapping it in Tor tools.
func wireGuardInvocation(action, configPath string, ninja bool) (string, []string, error) {
	if action != "up" && action != "down" {
		return "", nil, fmt.Errorf("invalid action %s", action)
	}
	if !ninja {
		return "wg-quick", []string{action, configPath}, nil
	}
	wrapper, err := resolveTorWrapper()
	if err != nil {
		return "", nil, err
	}
	return wrapper, []string{"wg-quick", action, configPath}, nil
}

// resolveTorWrapper finds a Tor-aware wrapper to launch wg-quick under.
func resolveTorWrapper() (string, error) {
	for _, candidate := range []string{"torsocks", "torify"} {
		if _, err := exec.LookPath(candidate); err == nil {
			return candidate, nil
		}
	}
	return "", fmt.Errorf("ninja mode requires either torsocks or torify in PATH")
}
